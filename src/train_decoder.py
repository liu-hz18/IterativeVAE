...
def train(epoch, model, dataloader, optimizer, scheduler):
    ...
    for i, (src, tgt_prev, tgt_gold, src_lens, tgt_lens) in tqdm(enumerate(dataloader, 0), desc='train', total=len(opensub_dataset)//opt.realbatch):
        out_seqs, loss, re_losses, kl_losses, out_lens, ntokens, alphas, betas = model(
        ...
        re_losses = [format(re_loss.item() / ntokens, '.4f') for re_loss in re_losses]
        kl_losses = [format(kl_loss.item() / ntokens, '.4f') for kl_loss in kl_losses]
        # calculate metrics
